...
LD_FLAGS = -X github.com/go-vela/vela-hugo/version.Commit=${GITHUB_SHA} -X github.com/go-vela/vela-hugo/version.Date=${BUILD_DATE} -X github.com/go-vela/vela-hugo/version.Go=${GOLANG_VERSION} -X github.com/go-vela/vela-hugo/version.Tag=${GITHUB_TAG}
...
build:
	@echo
	@echo "### Building release/vela-hugo binary"
	GOOS=linux CGO_ENABLED=0 \
		go build -a \
		-ldflags '${LD_FLAGS}' \
		-o release/vela-hugo \
		github.com/go-vela/vela-hugo/cmd/vela-hugo
...
build-static:
	@echo
	@echo "### Building static release/vela-hugo binary"
	GOOS=linux CGO_ENABLED=0 \
		go build -a \
...
build-static-ci:
	@echo
	@echo "### Building CI static release/vela-hugo binary"
	@go build -a \
		-ldflags '-s -w -extldflags "-static" ${LD_FLAGS}' \
		-o release/vela-hugo \
		github.com/go-vela/vela-hugo/cmd/vela-hugo
...
docker-build:
	@echo
	@echo "### Building vela-hugo:local image"
...
docker-test:
	@echo
	@echo "### Testing vela-hugo:local image"
	@docker run --rm \
		vela-hugo:local
...
docker-run:
	@echo
	@echo "### Executing vela-hugo:local image"
	@docker run --rm \
